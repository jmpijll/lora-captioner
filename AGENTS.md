...
## Project-Specific: LoRA Captioner

### Commit Protocol
- Commit after each meaningful change
- Use conventional commit messages: `feat:`, `fix:`, `docs:`, `refactor:`, `test:`
- Push to GitHub after every commit to maintain changelog

### Model Selection
- Primary model: `MiaoshouAI/Florence-2-large-PromptGen-v1.5`
- Rationale: Low VRAM (~1-2GB), purpose-built for LoRA training captions
- See `docs/research/vllm-models-comparison.md` for alternatives

### LoRA Type Handling
Three distinct captioning strategies:
1. **Character**: Caption everything EXCEPT core character features
2. **Style**: Minimal captions OR full descriptions (no middle ground)
3. **Concept**: Caption context, not the concept itself
See `docs/research/captioning-strategies.md` for details

### Testing Approach
- Always test with at least one image of each supported format
- Verify caption output makes sense for the LoRA type
- Test both GPU and CPU codepaths

### Documentation
- Update PLAN.md as features complete
- Document CLI changes in README.md immediately
